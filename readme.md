...
| 3 | Mixed content | Page is served over `https://` but pulled `http://maps.googleapis.com`, `http://ajax.googleapis.com`, and a Twitter meta image over plain HTTP - blocked/warned by modern browsers | Every `http://` reference changed to `https://`. |
